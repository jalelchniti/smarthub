...
const emailConfig = {
  ...
  auth: {
    ...
    pass: 'PASTE_YOUR_16_CHAR_APP_PASSWORD_HERE'   // Replace with the app password you just generated
  }
};
...
export const sendBookingNotification = functions.database
  .ref('/bookings/{bookingId}')
  .onCreate(async (snapshot, context) => {
    try {
      const bookingId = context.params.bookingId;
      const bookingData: BookingData = snapshot.val();
      ...
      const roomData: RoomData = roomSnapshot.val();
...
async function sendEmailNotification(booking: BookingData, room: RoomData): Promise<void> {
...
function generateHTMLEmail(booking: BookingData, room: RoomData, formattedDate: string, bookingPeriodText: string): string {
  return `
...
function generateTextEmail(booking: BookingData, room: RoomData, formattedDate: string, bookingPeriodText: string): string {
  return `
...
function formatBookingPeriod(period: string): string {
  switch (period) {
